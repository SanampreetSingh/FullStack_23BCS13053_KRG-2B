...
export default function CreateClass() {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      // sanitize and convert numeric fields to numbers to match backend expectations
      const payload = {
        ...data,
        ...
        subjects: (data.subjects || []).map((sub) => ({
          name: sub.name?.trim(),
          type: sub.type,
          credits: sub.credits === "" ? null : Number(sub.credits),
          internal: sub.internal === "" ? null : Number(sub.internal),
          external: sub.external === "" ? null : Number(sub.external),
          activities: (sub.activities || []).map((act) => ({
            name: act.name?.trim(),
            total: act.total === "" ? null : Number(act.total),
          })),
        })),
      };

      const res = await axios.post(`${backendURL}/class/`, payload);
      setMessage("✅ Class created successfully!");
      reset();
    } catch (error) {
      console.error(error);
      // backend returns plain string messages; show them if present
      const serverMsg = error.response?.data || error.message || "";
      setMessage("❌ Error creating class. " + serverMsg);
    } finally {
      setLoading(false);
    }
  };
