...
const Timetable = () => {
    ...
    const fetchClasses = async () => {
      try {
        ...
        if (response.data.length > 0) {
          ...
          setSelectedClassId(getId(firstClass));
  ...
  // Fetch timetable by class name
  useEffect(() => {
    const fetchTimetable = async () => {
      if (!selectedClassName) return;
      try {
        const response = await axios.get(`${backendURL}/class/timetable/${selectedClassName}`);
        ...
        (timetableData.days || []).forEach((day) => {
          // ensure each slot has the expected fields (time, subject, teacher, room)
          mapDays[day.name] = (day.slots || []).map((s) => ({
            time: s.time,
            subject: s.subject ?? "---",
            teacher: s.teacher ?? "---",
            room: s.room ?? "---",
  ...
  // Handle class select (set both ID and name)
  const handleClassSelect = (e) => {
    const selectedId = e.target.value;
    const getId = (obj) => obj?._id || obj?.id;
    const selected = classes.find((c) => getId(c) === selectedId);
    if (selected) {
      setSelectedClassId(getId(selected));
      setSelectedClassName(selected.className);
    }
  };
  ...
  // 🔹 Save timetable using class ID
  const saveTimetable = async () => {
    try {
      await axios.put(`${backendURL}/class/updatetimetable`, {
        classRef: selectedClassId, // backend expects class ID here
    ...
    <div className="w-full flex flex-col items-center gap-12 p-6">
      ...
      <div className="w-full max-w-6xl bg-white p-6 rounded-xl shadow-lg flex flex-col gap-6">
        ...
        <h2 className="text-3xl font-bold text-[#292321]">
          {selectedClassName} - {days[selectedDay]}
        </h2>
