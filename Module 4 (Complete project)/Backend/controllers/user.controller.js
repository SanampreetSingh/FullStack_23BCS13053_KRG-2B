...
exports.approveWaitingUser = async (req, res) => {
  try {
    ...
    const classDoc = await Class.findOne({ className: userToApprove.className });
    console.log("done");
    if (!classDoc) {
      // ❌ Class not found → remove waiting user to keep DB clean
      await waitinguser.deleteOne({ _id: id });
      return res.status(400).json({ message: "Class not found. Waiting user removed." });
    }
    ...
    // 4️⃣ Add student at the end of existing students (no overwrite)
    ...
    const newUser = new User({
      ...
      className: userToApprove.className,
      role: "user",
    });

    await newUser.save();

    // 6️⃣ Delete the waiting user after successful creation
    await waitinguser.deleteOne({ _id: id });

    res.status(200).json({ message: "Waiting user approved successfully" });
  } catch (err) {
    console.error("Error approving waiting user:", err);
    res.status(500).json({ message: "Internal server error" });
  }
};
